            ...
            if block.is_transaction_block():
                if block.transactions_generator is not None:
                    ...
                    npc_result = get_name_puzzle_conditions(
                        block_generator, min(self.constants.MAX_BLOCK_COST_CLVM, block.transactions_info.costi), cost_per_byte=self.constants.COST_PER_BYTE, safe_mode=False
                    )
        ...
        if block.is_transaction_block():
            if block.transactions_generator is not None:
                if npc_result is None:
                    ...
                    npc_result = get_name_puzzle_conditions(block_generator, self.constants.MAX_BLOCK_COST_CLVM, cost_per_byte=self.constants.COST_PER_BYTE, safe_mode=False)
                tx_removals, tx_additions = tx_removals_and_additions(npc_result.npc_list)
    ...
    async def validate_unfinished_block(
        ...
        if block.transactions_generator is not None:
            ...
            if block_generator is None:
                return PreValidationResult(uint16(Err.GENERATOR_REF_HAS_NO_GENERATOR.value), None, None)
            npc_result = get_name_puzzle_conditions(
                block_generator, min(self.constants.MAX_BLOCK_COST_CLVM, block.transactions_info.cost), cost_per_byte=self.constants.COST_PER_BYTE, safe_mode=False
            )
